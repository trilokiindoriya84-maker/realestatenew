...
export default function EditPropertyPage() {
  ...
  const propertyId = params.uniqueId as string;
  ...
  const loadProperty = async (uniqueId: string) => {
    try {
      const response = await api.get(`/properties/view/${uniqueId}`);
      const property = response.data;
      
      // Store existing documents separately
      setExistingDocuments({
        propertyPhotos: property.propertyPhotos || [],
        ownership: property.ownershipDocs || [],
        saleDeed: property.saleDeedDocs || [],
        khasra: property.khasraDocs || [],
        approvedMap: property.approvedMapDocs || [],
        encumbrance: property.encumbranceDocs || [],
        identityProof: property.identityProofDocs || [],
      });
      ...
      setFormData({
        ...
        propertyDescription: property.propertyDescription || '', // New field
        ...
        frontRoadWidth: property.frontRoadWidth || '',
        roadWidthUnit: property.roadWidthUnit || 'feet',
        propertyFacing: property.propertyFacing || '',
        openSides: property.openSides || '',
        // Road widths for each side
        eastRoadWidth: property.eastRoadWidth || '',
        westRoadWidth: property.westRoadWidth || '',
        northRoadWidth: property.northRoadWidth || '',
        southRoadWidth: property.southRoadWidth || '',
        ...
        circleRate: property.circleRate || '',
        documents: {
          propertyPhotos: [],
          ownership: [],
          saleDeed: [],
  ...
  const validateStep = (step: number): Record<string, string> => {
    ...
    if (step === 4) {
      // Step 4: Documents - check both new uploads and existing documents
      const totalPhotos = formData.documents.propertyPhotos.length + existingDocuments.propertyPhotos.length;
      const totalOwnership = formData.documents.ownership.length + existingDocuments.ownership.length;
      const totalSaleDeed = formData.documents.saleDeed.length + existingDocuments.saleDeed.length;
      const totalIdentityProof = formData.documents.identityProof.length + existingDocuments.identityProof.length;
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      // If we're on step 4 (documents), upload ONLY NEW files
      if (currentStep === 4) {
        ...
        dataToSave.propertyPhotos = [...existingDocuments.propertyPhotos, ...(uploadedDocs.propertyPhotos || [])];
        dataToSave.ownershipDocs = [...existingDocuments.ownership, ...(uploadedDocs.ownership || [])];
        dataToSave.saleDeedDocs = [...existingDocuments.saleDeed, ...(uploadedDocs.saleDeed || [])];
        dataToSave.khasraDocs = [...existingDocuments.khasra, ...(uploadedDocs.khasra || [])];
        dataToSave.approvedMapDocs = [...existingDocuments.approvedMap, ...(uploadedDocs.approvedMap || [])];
        dataToSave.encumbranceDocs = [...existingDocuments.encumbrance, ...(uploadedDocs.encumbrance || [])];
        dataToSave.identityProofDocs = [...existingDocuments.identityProof, ...(uploadedDocs.identityProof || [])];
        
        // Update existingDocuments state with merged data
        setExistingDocuments({
          propertyPhotos: dataToSave.propertyPhotos,
          ownership: dataToSave.ownershipDocs,
          saleDeed: dataToSave.saleDeedDocs,
          khasra: dataToSave.khasraDocs,
          approvedMap: dataToSave.approvedMapDocs,
          encumbrance: dataToSave.encumbranceDocs,
          identityProof: dataToSave.identityProofDocs,
        });
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              {currentStep === 4 && <Step4 formData={formData} setFormData={setFormData} errors={errors} existingDocuments={existingDocuments} setExistingDocuments={setExistingDocuments} />}
...
function Step1({ formData, setFormData, errors }: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Search Property Address <span className="text-red-500">*</span>
        </label>
        <MapboxAddressSearch
        ...
        <p className="text-xs text-gray-500 mt-1">
          Search and select your exact property location for better visibility to buyers
        </p>
      </div>
...
function Step4({ formData, setFormData, errors, existingDocuments, setExistingDocuments }: any) {
  ...
  const removeExistingDocument = (key: string, url: string) => {
    if (!confirm('Are you sure you want to remove this document?')) return;
    
    const updatedDocs = existingDocuments[key].filter((docUrl: string) => docUrl !== url);
    setExistingDocuments({
      ...existingDocuments,
      [key]: updatedDocs
    });
      ...
      <div className="space-y-4">
        {documentTypes.map((doc) => (
          <div key={doc.key} className={`border rounded-lg p-4 ${doc.key === 'propertyPhotos' ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
            ...
            {existingDocuments && existingDocuments[doc.key] && existingDocuments[doc.key].length > 0 && (
              <div className="mt-3 space-y-2">
                <p className="text-sm font-medium text-blue-600">
                  ✓ {existingDocuments[doc.key].length} existing document(s)
                </p>
                <div className="space-y-1">
                  {existingDocuments[doc.key].map((url: string, index: number) => (
                    <div key={index} className="flex items-center justify-between bg-blue-50 px-3 py-2 rounded border border-blue-200">
                      <a 
                        href={url} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="text-sm text-blue-700 hover:text-blue-900 truncate flex-1 underline"
                      >
                        {doc.key === 'propertyPhotos' ? `Photo ${index + 1}` : `Document ${index + 1}`}
                      </a>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-blue-600 px-2 py-1 bg-blue-100 rounded">Uploaded</span>
                        <button
                          type="button"
                          onClick={() => removeExistingDocument(doc.key, url)}
