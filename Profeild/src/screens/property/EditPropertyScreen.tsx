...
export default function EditPropertyScreen() {
  ...
  const loadProperty = async () => {
    try {
      ...
      const property = await propertyService.getPropertyForEdit(propertyId);
  ...
  const validateStep = (step: number): Record<string, string> => {
    ...
    if (step === 4) {
      // Step 4: Documents - check existing documents
      const totalPhotos = formData.documents.propertyPhotos.length;
      const totalOwnership = formData.documents.ownership.length;
      const totalSaleDeed = formData.documents.saleDeed.length;
      const totalIdentityProof = formData.documents.identityProof.length;
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      await propertyService.updateProperty(propertyId, dataToSave);

      // Upload documents if we're on step 4
      if (currentStep === 4) {
        ...
        for (const [docType, files] of Object.entries(formData.documents)) {
          if (files && files.length > 0) {
            ...
            if (newFiles.length > 0) {
              uploadPromises.push(
                propertyService.uploadPropertyDocuments(propertyId, docType, newFiles)
              );
            }
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        'Property updated and submitted for verification successfully!',
        [
          {
            text: 'OK',
